...
def getChars(matrix):
    ...
    for line in matrix:
        for i in line:
            if i!='.':
                chars.append(i)
    #for problem 2 sort the list and any that dont have a duplicate, remove so that they
    #are not included in the count because we are now counting the antennas
    chars.sort()
...
def getem(mark1, mark2, height, width):
    ...
    if mark1[1]<mark2[1]: 
        ...
        #bottomside
        xbot=mark2[1]+horiz
        while ybot<=height and xbot<=width:
            node=(ybot, xbot)
            anodes.append(node)
            ybot+=vert
            xbot+=horiz
